...
marker it produces is visible both in the transcript and in the worker list, where it
is what leaves both jobs blocked:
...
list and into the selected worker's header). Quotes below therefore differ from the
MVP-era chrome described earlier in this document; the behaviour they evidence is
unchanged.

The left column of `ui-03`, rows 9-15, contiguous:

```text

 mgr  Nothing to recover. Nothing needs you. 0 worker(s)
 still working.

 mgr  Started ENG-421 on the complete-ticket workflow. Step
 1: plan-feature on ENG-421 · planner.

```

and the worker list below it:

```text
 Workers (2) · 2 need you  ·  auto-advance on
 ! ENG-421 planner  [NEEDS DECISION] Choose the legacy-write s
 ! ENG-999 planner  [NEEDS DECISION] Choose the legacy-write s
```

with the selected worker's header in the right pane (the same row, right of the rule):

```text
 Manager                                                      │ ENG-999 · planner · blocked · planning · default model
```

Both jobs are blocked on a decision, both carry the `!` marker, and the header of the
selected worker carries the detail the list no longer repeats.
