...
class TestWorkflowsCommand:
    def test_it_lists_every_routable_workflow(self, csm_home, capsys):
        assert main(["workflows"]) == 0
        printed = capsys.readouterr().out
        for name in ("complete-ticket", "ask-question", "independent-review", "smoke-test"):
            assert name in printed

    def test_it_says_which_workflows_are_composite(self, csm_home, capsys):
        main(["workflows"])
        line = next(
    ...
    def test_each_workflow_is_one_line(self, csm_home, capsys):
    ...
    def test_a_user_workflow_appears_alongside_the_builtins(self, csm_home, isolated_workflows, capsys):
...
class TestConfigCommand:
    def test_it_prints_where_everything_lives(self, csm_home, capsys):
        assert main(["config"]) == 0
        printed = capsys.readouterr().out
        for label in ("config file", "data directory", "database", "worktree root"):
            assert label in printed
        assert str(csm_home) in printed

    def test_it_prints_the_effective_configuration(self, csm_home, capsys):
        main(["config"])
        printed = capsys.readouterr().out
